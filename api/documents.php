...
try {
    ...
    elseif($action==='upload'){
        ...
        if(!in_array($ext,$allowed)) {
            ob_end_clean();
            json_out(['success'=>false,'message'=>'Tipo non ammesso'],400);
        }
        ...
        if(isset($_POST['category']) && is_pro()){ 
            ...
            $st->execute(); 
            $rr=$st->get_result(); 
            if($row=$rr->fetch_assoc()) $label_id=$row['id']; 
        }
        ...
        $st->bind_param("iissis",$user['id'],$label_id,$f['name'],$f['type'],$f['size'],$doc_id); 
        $st->execute();
        ob_end_clean();
