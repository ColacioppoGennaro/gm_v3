/**
 * assets/js/ui.js (CORRETTO)
 * ✅ Filtri Area/Tipo nel WIDGET EVENTI
 * ✅ SOLO Categoria nella sezione Documenti
 */
...
function dashboardSection(isPro, maxDocs, maxChat, maxSize) {
  return `<section data-page="dashboard">
    ...
    <!-- ✅ DOCUMENTI: SOLO Categoria (visibile solo PRO) -->
    <div class="card">
...
function bindEvents() {
  if (window.S.view === 'login') {
    ...
    // ✅ Filtri Eventi (Area/Tipo)
    document.getElementById('filterEventArea')?.addEventListener('change', (e) => {
      window.S.filterEventArea = e.target.value;
      renderEventsWidget();
    });
    
    document.getElementById('filterEventTipo')?.addEventListener('change', (e) => {
      window.S.filterEventTipo = e.target.value;
      renderEventsWidget();
    });
    
    // ✅ Filtro Documenti (SOLO Categoria)
...
export function showPage(pageName) {
  ...
  if (pageName === 'dashboard') {
    ...
    loadEventFiltersData(); // ✅ Carica aree/tipi per TUTTI (anche FREE)
    renderEventsWidget();
...
/**
 * ✅ Carica dati per filtri Eventi (Area/Tipo)
 */
async function loadEventFiltersData() {
  try {
    ...
    if (tipi.success) {
      const filterTipo = document.getElementById('filterEventTipo');
      if (filterTipo) {
    ...
    console.error('Errore caricamento filtri eventi:', error);
  }
}
